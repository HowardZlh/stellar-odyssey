...
分析问题、给方案、做取舍时，按下列顺序组织论点。**硬约束不参与排序，始终在框架之上**：科学准确性、支持文案边界、CF 零费用、Git 工作流、测试覆盖 gate、移动端兼容等既有强制规则，任何视角都不得以"用户/运营需要"为由放松。

- **默认顺序（判断题）：用户 → 运营 → 专业性。** 适用于文案、功能取舍、选题、排期、UI 口径、交互设计等正确答案取决于价值判断的问题。先问"用户能感知到什么、体验是否更好"，再问"转化/传播/维护成本/排期是否划算"，最后才是实现方式是否优雅。
- **专业性问题：专业性 → 用户 → 运营。** 判据：正确答案由客观事实或规范决定——科学数据与物理、代码正确性、安全与合规、CF 免费额度、测试覆盖、性能指标。先把事实与规范说清，再谈对用户的影响，最后谈运营取舍。
- **定义**：用户 = 终端用户可感知的价值与体验（具体人群以下节「用户画像」为准）；运营 = 转化、传播、维护成本、排期与商业口径；专业性 = 科学准确、工程质量、安全合规。
- **拿不准归类时**：先按专业性判据自检一次（"这道题有客观正确答案吗？"），有则走专业性顺序，无则走默认顺序；在输出中点明所用顺序，便于用户纠偏。

## 用户画像（强制 · 分析基准）

完整画像、证据索引与验证状态在内部文档 `docs/internal/planning/USER_PERSONA.md`（不入库）。**分析任何涉及"用户"维度的问题前必读该文件**；本节只列最小强制条文：
...
- **画像是假设不是数据**：输出中一律写"按项目画像假设"，禁止写"数据显示用户……"；当验证数据源（CF Web Analytics、漏斗回读、内容台账、线索表）出现冲突读数，Agent 应主动提出修订内部文件与本节，而非沿用旧假设。
...
发布须由用户明确指示，Agent 不得自行发起。收到发布指示后按以下惯例执行（v0.1.6/v0.1.7/v0.1.8 先例）：

1. **发布分支**：从最新 `main` 切出 `release/vX.Y.Z`（分支创建照常先征求用户确认）；**禁止直接在 main 提交发布 commit**。
2. **发布 commit**（单个 commit 包含全部三处改动）：
   - `npm version X.Y.Z --no-git-tag-version`（package.json + package-lock.json 同步升版）；
   - CHANGELOG `[Unreleased]` 区段整体归档为 `[X.Y.Z] - 日期`（保留空的 `[Unreleased]` 标题）；
...
- **统一"支持即解锁"口径（Z 迭代 M3，取代原 U 迭代双轨隔离）**：解锁与赞助入口互通——解锁入口允许出现在 /donate 页、ContactBadge、README 赞助节（`.github/FUNDING.yml` 除外：GitHub Sponsor 按钮仅支持赞助平台链接，保持现状，D-z10）；**渠道顺序全站同口径**：爱发电（推荐 · 订单号自动兑换即时解锁）→ 支付宝扫码（自动发码即时解锁）→ 面包多（备选 · 扫码即付无需注册 · 订单号自动兑换）→ Ko-fi（海外备选 · 人工核验 · token 经 Email 发送），/unlock 与 /donate 两页渲染顺序有断言测试锁定；**微信赞赏码渠道已下线**（不在任何页面渲染；`DonationPlatformId` / `UNLOCK_CHANNELS` 保留 `wechat` 仅为历史贡献者记录与已签发 token 兼容，勿删）；付款 modal 只在 /unlock 页（/donate 爱发电与支付宝面板均为引导跳转）；Ko-fi 人工渠道兑换邮件经 `utils/redeemMail.ts` 拼装 mailto（`unlock.emailSubject` + `unlock.mailTplBody`），禁止第二份副本
