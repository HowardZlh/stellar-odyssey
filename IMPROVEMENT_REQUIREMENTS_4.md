...
> **文档版本**: 1.4（§R4-4 登记"自适应质量升降档判据已被 L4 频闪 P0 修复修订：降档阈值 55→52 + 新增 3s 换档驻留"，见 §R4-4 修订块；1.3：R4-1～R4-24 全部 ✅ 已完成，R4 迭代收尾）
...
> **⚠ 升降档判据已被 L4 频闪 P0 修复修订**（2026-07，用户反馈缺陷；登记先例参照 R5-8 在 R2-4 的修订块）：R5 负载（近观星系 + R5-2 体积尘埃盘 + R5-3 目录点云 + R5-6 费米气泡 + Bloom）下帧耗时骑在原 55 降档阈值上，`decideTier` 出现 mid↔low **极限环振荡**（降档 <55 即时 ↔ 升档 ≥58 持续 5s 反复触发）——uSteps 36↔24 斜坡 + RT resolutionScale 切换经 Bloom mip 链放大为盘面雾光明暗频闪（叠加透明层 draw 顺序翻转缺陷，见 `utils/universeRenderOrder` 注册表登记）。修订两重阻尼：a) **降档阈值 55→52**（`QUALITY_DOWNGRADE_FPS`，迟滞带 3→6 FPS——骑 55 场景落入迟滞带稳定持档）；b) **新增 3s 换档驻留**（`QUALITY_CHANGE_DWELL_MS`：任何换档后 3s 内不再换档；驻留期升档达标累计照常推进、届满即结算；创建时刻视作换档起点 = 挂载后 3s 观察期）。升档判据（≥58 连续 5s、起点回溯窗口起点）不变，驻留 3s < 判据 5s 故"恢复后 ~5s 升档"验收时序零回退；预览页强制档位滑杆（R4-24 验收项）语义不变。下文 4.1/4.2 的 55 阈值与换档时序描述仅存档 R4-4 交付时语义；单测锚定已同步迁移（`adaptiveQualityR44` 增驻留时序 + 骑阈值不振荡回归例）。
...
- ✅ 新建 `src/components/Scene/volumetric/BlackHoleLensed.tsx` + 纯逻辑 `src/utils/blackHoleLensing.ts`：包围球（半径 14 r_s 球壳，BackSide）raymarching——
  - ✅ 光线弯曲：Schwarzschild 二阶近似（**不做全数值测地线积分**）。实现差异登记：取"弱场积分核空间分布 + 二阶闭式总量预算"两级结构——总偏转角 α(b) = 2·r_s/b + (15π/16)·(r_s/b)²（一阶即弱场式 4GM/(c²b)；二阶系数 Keeton & Petters 2005，即近光子球增强项），逐步弯折用弱场核 dα/ds = r_s·b/r³ 并以 α(b) 为硬预算（出射方向与闭式严格一致；含增强项的分布核沿弯折路径评估会正反馈过弯、把撞击边界推至 b≈3.9 r_s，实测登记弃用）；适用域 b ≳ 2 r_s 定量可靠，b→b_crit 真解对数发散域为有限近似（登记）
  - ✅ 视界内黑（撞击 r ≤ 1.05 r_s 终止为黑）+ 补充解析阴影判据 b < b_crit = 3√3/2·r_s 且起步朝向中心（Schwarzschild 捕获截面精确结果 MTW §25.6；方向判据防近距观察者视野整片误黑，实现登记）；光子环（b ≈ 2.6 r_s 附近积累增亮：撞击参数选通核 × 光子球邻域驻留核沿程积累，CPU 验证峰值恰落 b=2.6、两侧 ≥2× 衰减）
  - ✅ 背景采样：弯曲后方向采样程序化星场 cubemap（128px/面，FNV-1a 确定性，O/B 蓝→M 红色板与 Starfield 同源；无贴图资产），背景恒星拖曳成弧；同一 cubemap 兼作 scene.background，包围球轮廓处切向弯曲趋零→内外连续无接缝
- ✅ 数值稳定：单步偏转钳 0.35 rad/出包围球提前终止/方向重归一化与零长度防护/预算硬钳；log depth 兼容（logdepthbuf include）+ 输出亮度硬钳 8.0；64 步 60 FPS（实测全部 4 距离档与 128 步档均 60 FPS）
- ✅ 预览页注册 `?body=blackhole-test`：滑杆三件（质量尺度 0.3–4×/相机距离/步数 16–128）；**交付后人工目检检查点**：光子环成形、背景弧状拖曳可辨，确认后方可启动 R4-12
- ✅ 纯逻辑单测 `blackHoleLensingR411`（33 例）：偏转角函数（弱场/二阶闭式/渐近）/撞击与阴影判定边界/CPU 参考光线追踪 5 条样例（弱场与中场偏转 = 闭式、近临界环发光、深入射撞击、近距外行光线不误黑）；shader 常数经模板插值与纯逻辑单点同源 + 同式系数一致性断言

### 11.2 验收标准

- ✅ 预览页可见清晰光子环 + 背景星弧状弯曲，绕行/推近连续无闪烁（4 距离档 40/20/8/4 截图登记：r411-dist-* / r411-orbit-*）
- ✅ 相机从远处推入至近光子球距离（质量尺度 4× + 距离 4 ≈ 2 r_s）画面无 NaN 黑块/白闪（2 r_s 处全视野入影为物理正确：真解阴影半张角 ~67° > 半视场 53°，登记）；全程 60 FPS（未触发降档）
- ✅ 偏转角/撞击判定单测；覆盖率 gate ≥90% 保持（语句 98.56%，blackHoleLensing.ts 99.42%）

## R4-12 黑洞引力透镜 ②：吸积盘物理

### 12.1 需求

- ✅ raymarch 内加入薄盘相交（盘平面 r ∈ [r_isco≈3r_s, ~12r_s]，几何薄盘，弯曲光线与盘面求交可多次相交——上下缘翻折成像的来源）。实现登记：弯折步进循环内做平面跨越检测（前后两步盘空间高度异号 → 线性插值求交 `planeCrossingLerp`，非解析求交）；盘姿态经 mat3 uniform `uDiskRot`（mesh 不旋转，保 R4-11 cubemap 采样约定）；前向透射合成——盘近不透明（每次交盘透射率 ×(1−a·0.92)，内外缘 smoothstep 软化 + 条纹微透明），交盘后背景/阴影按剩余透射率显示、光子环发光乘透射率（盘后被遮蔽）
- ✅ 盘温度分布 T(r) ∝ r^(−3/4)（Novikov-Thorne 近似，内缘截断登记：× [1−√(r_in/r)]^(1/4) 应力零边界项，峰值恰落 r = (49/36)·r_in、归一化闭式 NORM = (49/36)^(3/4)·7^(1/4)）→ `blackbodyRGB`（复用 R4-6 `starPhysics`）着色——`buildBlackbodyLutData` 预采样 [1,000, 16,000] K → 64 texel RGBA LUT 供 shader 查表（逐 texel 与 blackbodyRGB 一致单测断言）；峰值温度 7,200 K 为可视化压标档登记（真实内区 ~10⁷ K X 射线域，光学黑体色板不可表现）
- ✅ 多普勒束流：盘内开普勒速度场 β = √[(r_s/2)/(r−r_s)]（Schwarzschild 圆轨道局域速度精确式，ISCO 处恰 0.5c 单测锚定）→ 近侧蓝移增亮（δ³ 近似，δ = √(1−β²)/(1−β·cosθ)；频率积分真值 δ⁴ 差异登记）、远侧红移减暗；引力红移随 r 减小加深（近似式登记：g = √(1−r_s/r) 静止发射体→无穷远观察者式，亮度 ×g³、色温 ×g，忽略盘倾角与观察者位置次级项）；色温随 δ_eff × g 偏移入 LUT（近侧偏蓝白/远侧偏橙红）
- ✅ 盘面细节：径向流动噪声条纹（uTime 驱动差速旋转 ω ∝ r^(−3/2) 开普勒律形状，速率为可视化节奏登记；值噪声两八度 cos/sin 嵌入消 φ 接缝，径向频率高/角向低呈细密环状纹理）；亮度与 Bloom 联调不过曝（盘亮度基准 0.85 目验调参登记：2.6 档实测全盘 tonemap 洗白失去黑体色阶与束流对比）
- ✅ 预览页滑杆：盘倾角（0°=正视/90°=侧视）/内外缘半径（2–6 / 6–13 r_s，`clampDiskRadii` 防交叉）/束流强度（0–2：0 关闭/1 物理档 δ³/2 夸大；引力红移不随滑杆关闭登记）——合计 7 件 ≤8 上限；**人工目检检查点**：上下缘翻折 + 左右不对称亮度（Interstellar/EHT 观感）成立后方可启动 R4-13

### 12.2 验收标准

- ✅ 预览页：盘的上缘/下缘翻折像 + 近侧亮远侧暗不对称清晰可辨（正视 5°/斜视 60°/侧视 85° 3 角度截图登记 r412-face-on-5/oblique-60/edge-on-85；侧视远侧盘面折上阴影上方成拱 + 下缘二次像，束流 A/B 截图 r412-beam-0/1/2 不对称随滑杆增强；另登记内外缘滑杆 r412-disk-wide/narrow、条纹流动 2.5s 两帧 r412-flow-t0/t1、质量尺度 2× 推近 r412-near-push 无 NaN）
- ✅ 温度→颜色/速度→束流因子纯函数单测（与 shader 系数同式断言；新增 `blackHoleLensingR412` 36 例，R4-11 33 例零修改回归）；60 FPS（全部视角/操作满帧，占比 ≤1/2 满足）；覆盖率 gate ≥90% 保持（语句 98.58%，blackHoleLensing.ts 99.6%）

## R4-13 黑洞引力透镜 ③：场景接入（人马座 A* / 天鹅座 X-1）

### 13.1 需求

- ✅ `SpecialBodies.tsx` `BlackHole` 组件挂接 `useDetailLayer({ kind:'lensing' })`：跟随/飞往两黑洞且近观距离内激活 `BlackHoleLensed`（新组件 `Scene/BlackHoleLensedLayer.tsx` 消费 `utils/blackHoleScene.ts` 配置），与现有廉价 shader（吸积盘+光环）交叉淡出（shader 增量 uFade uniform + transparent，门控 opacity01 × 层级权重；廉价层 uOpacity ×(1−门控) 反向；renderOrder=2 晚于廉价盘绘制）；退出恢复、资源随卸载 dispose（材质/星场 cubemap/黑体 LUT）。**实现差异登记**：a) 透镜接管（门控 >0.5）后事件视界黑球停写颜色/深度——raymarch 阴影（b_crit≈2.6 r_s）大于黑球（1 r_s），保留黑球深度会切断盘前景翻折像；黑球 mesh 保留为点选 raycast 命中体；b) 门控判据修复（目验发现）：useDetailLayer 默认 focused 判据含 flyToBodyId（抵达后不清零）致 Esc 退出后无法释放，透镜层经 getFocused 注入改为仅按 followBodyId（requestFlyTo 同步置 followBodyId 运镜期门控不变；其余细节层默认判据零改动）
- ✅ 两黑洞参数区分（`BLACK_HOLE_LENSED_CONFIGS` 两份配置对象）：Sgr A*（大质量、盘暗弱偏橙红——峰值色温压标 7200 K×0.64≈4,600 K + 亮度 0.55 + 暖橙光子环；实际为射电/亚毫米波段亮度，黑体色档艺术化登记）/ 天鹅座 X-1（恒星级、盘亮偏蓝白——×1.36≈9,800 K + 亮度 1.15 + 冷蓝白环；shader 增量 uDiskTempScale uniform，R4-12 既有单测零修改）；伴星联动登记："如有则保留"条件不成立——现状 BlackHole 组件无伴星渲染，本阶段维持；质量差异经 visualRadiusLy 尺度体现（视界/环/盘世界尺寸 Sgr A* ≈2.3× Cyg X-1）；盘倾角两者同取 69.23°（= 廉价盘 rotation.x=−π/2.6 法线姿态，交叉淡出盘面对齐无跳变，单测断言）
- ✅ 尺度处理：视界渲染半径（= 透镜 r_s 世界长度）沿用现有 `visualRadiusLy` 压缩比例——即廉价 shader 黑球半径（视觉尺寸 ×0.32 单点同源；真实 r_s Sgr A*≈0.0013 ly 角尺寸不可辨，压缩登记）；包围球（14 r_s）小于两黑洞飞抵观察距离（67.2<90 / 29.1<39 场景单位，相机恒在球外，单测断言）；背景弯曲采样**取程序化星场近似**（登记：非场景 cubemap 快照——快照需每黑洞 6 面离屏渲染主场景，银心方向粒子峰值场景成本高且跟随中背景随相机变化需重拍；差异登记：球内被弯曲星场为程序化近似星点、与球外真实背景不逐星对应，球缘弯曲趋零 + 随机星点下不连续目验不可辨；两黑洞确定性种子不同）
- ✅ 性能：lensing 池容量 1（R4-2 既有）；自适应降级接入（R4-4 档位状态机复用，档位映射步数 64/48/32 = 基准 64×stepScale 经 clampLensingSteps，≤0.5s 平滑插值；透镜为不透明全分辨率 raymarch，RT 半分辨率通道不适用登记）；巡游切换进出 5 次无泄漏（无头 Chrome 实测堆恒稳 47 MB 零净增）

### 13.2 验收标准

- ✅ L3 巡游至人马座 A* / 天鹅座 X-1：飞抵后平滑过渡到透镜渲染（时序截图 r413-sgr-01～05 / r413-cyg-01～04：1200ms 廉价远景 → 3000/3600ms 交叉淡出 → 抵达透镜成像，对照现状登记升级幅度），光子环+盘翻折+束流不对称可辨（绕行截图 r413-sgr-1x / r413-cyg-1x；Sgr A* 暖橙 vs Cyg X-1 蓝白区分清晰）；Esc 退出恢复远景表现（r413-cyg-21/22 + r413-esc-01～04：0.5s 淡出恢复廉价 shader、重入重建正常）
- ✅ 60 FPS 不跌破 55（两站飞抵/绕行/退出实测均 60 FPS，未触发降档；自适应 mid 档待命）；进出 5 次 JS 堆回落登记：出/入采样 [[47,47]×5] MB 零净增（CDP 强制 GC 后采样；显存侧透镜层估算 384 KB 随卸载出账）
- ✅ L3 全序列回归无其他站点影响（r413-l3-01～06 其余站点截图正常、控制台零错误）；覆盖率 gate ≥90% 保持（全量 2389 用例/128 套件，语句 98.58%，blackHoleScene.ts 100%）

## R4-14 环状星云 M57 壳层体积模型

### 14.1 需求

- ✅ `nebulaVolume.ts` 新增 M57 密度场：三轴椭球壳层模型（O'Dell et al. 2013, ApJ 780, 26 形状参考，参数登记于文件头）——`makeM57Sampler`：半轴 0.58/0.48/0.38（a:b:c ≈ 1:0.83:0.66，极轴 +z 朝观察者，真实极轴 ~30° 倾斜由主场景环面姿态组承担登记）+ 赤道增密环/极向暗瓣（极角余弦平滑压暗至 0.22 档）+ 内腔近空（0.05 弱 OIII 充盈）+ 环壳 OIII 青绿 → 外缘 Hα/NII 红橙渐变（`m57ColorWeight01` 按椭球归一化半径分层，NII 与 Hα 合并单档登记；材质双色权重泛化：`uWeightInvRadii` 椭球归一化逐轴倒数，M42 默认 (1,1,1) 行为零回退）+ 外晕弱壳（×1.35/幅度 0.06，真实双层晕合并单层登记）；壳层解析式塑形、fBm 仅做扰动（壳半径 ±0.1 + 束状调制）；吸收通道恒零登记（无显著前景尘埃，单测断言）
- ✅ 中心白矮星亮点（复用 R4-6 色档：Teff≈125 kK 经 `blackbodyRGB` 表上限 50 kK 档偏蓝白，远紫外主导真实色感以表上限近似登记；星点 sprite 工具泛化 `buildStarSpriteTexture(tint)`，M42 蓝白默认零回退）；体积纹理 96³（预算登记：RG 双通道 ≈1.69 MB ≪128³ 的 4 MB）
- ✅ 主场景接入同 R4-8 模式（`useDetailLayer` volume 池容量 1，M42↔M57 切换 LRU 逐出；环面 billboard/中心白矮星网格 `m57BillboardFactor` 与 R2-7 交付的 +200 环向粒子/外晕壳 `m57NearLayerFactor` 在体积激活时 0.5s 交叉淡出，登记）。实现差异登记：a) `OrionVolumeLayer` 泛化为通用 `NebulaVolumeLayer`（M42/M57 共用同一接线，密度场/材质参数/内嵌星点经 `NebulaVolumeLayerConfig` 注入，R4-8 行为零回退单测断言配置等价）；b) M57 体积位姿参考 = 环壳缩放组（倾斜姿态 + 膨胀动画随动），包围盒边长 = 视觉尺寸 ×2.9（壳中面折算 ≈0.84× 视觉半径与 R2-7 环径 0.85× 衔接）
- ✅ 预览页注册 `?body=ring-nebula`（`OrionNebulaPreview` 泛化为 `NebulaVolumePreview`，滑杆 6 件：步数默认 48/密度/双色权重/亮度/质量档/抖动——无尘埃滑杆，吸收通道恒零登记）；确定性种子（FNV-1a `volumeSeed('ring-nebula')`，双次构建逐字节一致单测）；密度场关键点单测（赤道环 ≥2.5× 极向、内腔 ≈0.05 充盈档、外晕弱正值、吸收恒零）

### 14.2 验收标准

- ✅ 近观绕行：从正视"环"（暗腔 + 青绿内缘 + 红橙外缘 + 中心白矮星）到侧视"桶状/椭球壳"的视差变化真实可辨（预览页 r414a/r414b + 主场景 r414c 各 3 角度截图登记）——billboard 方案不可能呈现的核心升级点
- ✅ 与 M42 巡游连续切换 ×3：volume 池 LRU 逐出正常（烘焙打点交替 M42 ~490ms/M57 ~80ms，单挂载互斥、无双体积同挂）、强制 GC 后 JS 堆 49→50 MB 无增长趋势（无泄漏）；全程 60 FPS 保持、控制台零错误
- ✅ 密度场单测（`nebulaVolumeR414` 30 例）+ 回归全绿（R4-7 31 例 / R4-8 22 例零修改，全量 2419 例/129 套件）；覆盖率 gate ≥90% 保持（语句 98.56%）

## R4-15 马头星云吸收体积（暗云 + 背景发射幕）

### 15.1 需求

- ✅ `nebulaVolume.ts` 新增马头密度场：以**吸收为主**的暗分子云柱（`makeHorseheadSampler`：马头轮廓 = 5 个椭球 SDF 平滑并——颈柱/头部/吻部/鬃丘 + 底部暗云堤（B33 从 Lynds 1630 云堤伸出的形态近似；省 token 约定"3–4 个椭球"为补底部云堤取 5 个，登记）+ 3 八度 fBm 边缘侵蚀（轮廓半径扰动 ±0.09 + 表层湍流阈值化碎散云缘，深处不受侵蚀）+ 盒边缘软窗防切边，Hubble 公版图像轮廓参考、不逐像素贴合照片登记），马头轮廓内发射通道近零（≤0.02 单测断言）；纹理 96³（RG ≈1.69 MB 预算登记）
- ✅ 背景发射幕：二选一取**低密度大尺度发射层**（登记）——IC 434 红色发射幕烘焙进体积后半域（z 向软窗剖面 + 横向软窗防盒边切边 + fBm 云絮），剪影 = 吸收柱按透射率物理遮挡幕布（侧向绕行可见云柱纵深，预览页黑底亦自含可辨）；补充登记：主场景既有背景 billboard **保留**并在体积激活时部分减淡（`horseheadCurtainFactor` ×(1−0.35·vol01)）作幕布远景延伸，体积盒外围无幕布断边；IC 434 Hα 单色登记（weightBias=+1 恒取 colorHa 红棕 #c9503a）
- ✅ 主场景接入同 R4-8 模式（`DarkNebula` 挂接 `useDetailLayer({kind:'volume'})` + 泛化 `NebulaVolumeLayer` 复用，无内嵌星点登记：B33 冷分子云无点源；包围盒 = 视觉尺寸 ×2.0 与剪影 billboard 组衔接登记；volume 池容量 1，M42/M57/马头切换 LRU 逐出）；R2-7 交付的 2 视差发射层 + 3 前景暗云团在体积激活时交叉淡出（`horseheadNearLayerFactor` = near01×(1−vol01)，vol01=0 行为零回退单测锚定，登记）；补充登记：3 块前景暗云柱剪影 billboard 同步淡出（volDim ×(1−vol01)，体积吸收柱接管剪影）
- ✅ 预览页注册 `?body=horsehead`（§ 指定短名，与天体 id `horsehead-nebula` 差异登记；滑杆 6 件：步数 48/发射幕密度/尘埃吸收/亮度/质量档/抖动，无双色权重滑杆——Hα 单色登记）；密度场单测 `nebulaVolumeR415`（26 例：轮廓内吸收 ≥0.5/轮廓外 ≤0.05、发射通道 ≤0.02 断言 + 发射幕后方 ≥0.04 前方近零 + 确定性/spec/交叉淡出/配置/注册）

### 15.2 验收标准

- ✅ 近观绕行：马头剪影随视角变化呈现真实体积轮廓（预览页正面剪影清晰——颈柱/头部/吻部/鬃丘衬红幕可辨、45° 斜视/侧视云柱纵深真实；主场景近观绕行 3 角度立体连续，截图 r415a-00～03 / r415c-00～03 登记）
- ✅ 巡游连续切换回归、无泄漏（M57↔马头 ×3 切换 LRU 逐出、每次返回重新烘焙打点一致 ~190ms/8 块/最大单块 24.6ms <100ms 卡顿约束，强制 GC 后 JS 堆 50→50 MB）；60 FPS 保持（全程未触发降档）；覆盖率 gate ≥90% 保持（语句 98.58%，全量 2445 用例/130 套件）

## R4-16 蟹状星云丝状结构 + PWN 环面/喷流

### 16.1 需求

- ✅ `nebulaVolume.ts` 新增蟹状密度场：外围丝状网络（方向场扭曲的密度脊，Hα 红橙丝 + 内部 OIII 青色弥散，Hubble 形态参考登记）+ 整体椭球包络。实现差异登记：a) 丝状网络按省 token 约定取 12 条参数化曲线骨架（`crabFilamentPolylines`：单位球面切向随机游走长弧 = 笼壳丝网形态——任意视角投影呈网状长丝而非径向辐条投影团块，目验迭代定型）沿线增密（线段距离软衰减密度脊）+ 采样域经湍流值方向场扭曲 ±0.06 + 沿线束状调制亮结；骨架距离经包围球平方距离早退 + 扁平段数组零分配，128³ 烘焙墙钟 ~300ms 与 M42 同量级；b) 双色权重 `crabColorWeight01` 按椭球归一化半径分层（内 OIII 青弥散 → 外 Hα 红橙丝）——真实丝网红/青逐丝交织按内外分区纯径向近似登记；c) 吸收通道恒零登记（蟹状无显著前景尘埃，单测断言）；纹理 128³（RG 4 MB）
- ✅ PWN（脉冲星风云）内核：Chandra 形态参考的赤道环面（torus）+ 极向双喷流——环面用 shader 环形发射体（蓝白同步辐射色 #bfe0ff：径向剖面 = 纯函数 `crabTorusIntensity` 的 GLSL 镜像，主环 0.62 + 内环 0.28 双环形态 + 中心弱辉 + wisp 细纹缓慢外流；环面平面增强项由平面几何承载 height01=0、CPU 纯函数含高度衰减供验收断言登记），喷流复用/参数化 `ExtragalacticObjects.tsx` `RelativisticJet` 锥体 shader（复用登记：组件导出 + 新增可选 `getWeight` 权重覆写注入 L3 近观权重、lengthUnits 参数化缩小——类星体/M87 既有消费零改动零回退；方向 ±y 自转轴与射束进动轴一致）。尺度夸大登记：真实环面 ~0.5 ly ≪ 星云 ~11 ly，可视化按环面半径 0.30× 视觉半径（~6×）保近观可辨
- ✅ 与现有 `PulsarRemnant`（:1406）整合：射束扫描/脉冲节奏保留（`pulsarPulseIntensity`/`pulsarBeamAngle` 零改动，无头 Chrome 连拍复验 ~1.2s 周期峰值）；R2-7 的 +16 丝状云团在体积激活时交叉淡出（`crabNearLayerFactor` = near01×(1−vol01)；遗迹壳 billboard 同步经 `crabBaseLayerFactor` 淡出，vol01=0 时 = R2-7 近观减淡 45% 零回退）；脉冲星本体/射束与体积层深度关系正确（方案登记：体积合成三角形维持 depthTest=false——R4-8 结论沿用；本体/射束/环面/喷流绘制于合成之前按整盒透射率压暗，R4-7/R4-8 星点 sprite 先例；射束/脉冲闪烁按 `crabCoreBoostFactor` ×(1+0.6·vol01) 透射补偿保节奏观感）。补充登记：PWN 环面/喷流经近观门控挂载、不随体积淡出（蓝白内核为体积双色之外的第三层）
- ✅ 主场景接入同 R4-8 模式（`useDetailLayer({kind:'volume'})` release-on-exit + 泛化 `NebulaVolumeLayer` 复用，volume 池容量 1 与 M42/M57/马头共池 LRU；包围盒 = 视觉尺寸 ×2.6，billboard 膨胀动画不随动体积登记——激活时已交叉淡出）；预览页注册 `?body=crab-pulsar`（滑杆 6 件：步数 64/密度/双色权重/亮度/质量档/抖动，无尘埃滑杆登记；预览页仅体积层登记——PWN 环面/喷流为主场景近观组件经主场景截图验收）；密度场单测（`nebulaVolumeR416` 33 例：丝状脊密度 ≥2.5× 弥散区、环面平面增强——同径向半径平面内 > 离面 ×2 断言）

### 16.2 验收标准

- ✅ 近观：外围红橙丝状网络 + 内部青色弥散 + 中心蓝白环面/双喷流三层结构可辨，绕行立体（预览页 r416a-00～03 + 主场景 r416c-00～03 各 3+ 角度截图登记：丝网长弧缠绕、青弥散内域、双环环面 + wisp、极向喷流线）；脉冲节奏不回退（连拍 24 帧中心亮度呈 ~1.2s 周期峰值 = 2.4s 自转周期 × 每转两次射束扫过，r416c-05/06 峰谷对照登记）
- ✅ 体积 + 环面 + 射束 + Bloom 组合 60 FPS 不跌破 55（实测全程 60 FPS，未触发降档）；LRU 回归无泄漏（蟹状↔M42 切换 ×3 烘焙打点交替 ~300ms/~490ms、单挂载互斥，强制 GC 后 JS 堆 51→52 MB 无增长趋势）；覆盖率 gate ≥90% 保持（语句 98.55%，全量 2478 用例/131 套件）
...
- ✅ `StellarSurface` 参宿四专属增强：低阶球谐（l ≤ 3）扰动叠加基础噪声（shader uShAmp/uShSpeed + GLSL `shPerturb`，CPU 镜像 `utils/stellarNearView.betelgeuseShPerturbation`——省 token 约定 4 项峰值归一实球谐基 l=1,2,2,3 固定系数展开，非通用球谐库）——大尺度不对称亮/暗斑（首项 l=1 偶极最重呈半球亮暗基调 + 高阶细分 2–3 斑块），缓变余弦系数驱动演化（视觉周期 46/61/74/88 s 登记，全部落在 40–90 s 区间、互质防拍频；VLTI/Montargès 2021 观感参考）；可选尘埃抛射暗斑事件**不实现**（非硬性，登记：球谐负半周期本身呈现大暗斑，已覆盖 2019-20 大变暗观感基调）；uShAmp=0 时其余 5 类恒星行为零回退
- ✅ 恒星近观通用点缀（全部 6 类恒星，新组件 `StarNearDress`）：近观距离内色球边缘辉光环（新贴图 `createChromosphereRingCanvas`，环峰值 limb 外 1.04× 半径；色温联动 `chromosphereRGB`——黑体基色向 Hα 红端混合、权重随 Teff 单调递减 ≥8,000 K 归零，线性混合近似登记）+ 相机中距衍射星芒 sprite（复用 `createDiffractionSpikeCanvas` 色温联动同色；距离窗口纯函数 `starSpikeWindow01` = smoothstep(2.2r, 4.0r, d) 近观推近淡出防遮挡 + 单测；边长 6.5× 半径长于辉光 sprite 半程防淹没，目验调参登记）；天狼星差异登记：A 星环+芒随互绕轨道、B 星仅环（常驻衍射芒线已有不叠加）；近观增量登记恒星站 0→2 / sirius 0→3（`STAR_NEAR_DRESS_SPRITE_COUNTS` 与 `NEAR_VIEW_PARTICLE_INCREMENTS` 单测同步防漂移）
- ✅ 预览页参宿四滑杆：球谐斑块幅度（0–1 默认 0.55）/球谐演化速度（0–6 默认 1），仅参宿四注册（其余恒星 shAmplitude=0 单测断言）；主场景经 `useNearViewGate`（useDetailLayer 薄包装，R2-7 同源阈值、release-on-exit）近观激活——星芒/色球环仅近观挂载、远景零开销，卸载即 dispose

### 18.2 验收标准

- ✅ 预览页参宿四：大尺度不对称斑块可辨且缓慢演化（真实速度间隔 30 s 两帧对比截图登记 r418-preview-bet-t0/t30——亮斑重分布、暗区迁移；幅度滑杆 0.55→0 斑块消失截图 r418-preview-bet-amp-0；30 s 全球面扰动 RMS 变化 >0.1 单测锚定），与其他恒星均匀颗粒观感形成区分（参宿七对照截图 r418-preview-rigel-uniform）
- ✅ L3 巡游恒星站近观：色球环/星芒出现与淡出平滑（5 站截图 r418b/r418d-near-* 登记——参宿四暖红环/热星蓝白环色温联动可辨；参宿四站滚轮推近星芒 ≤2.2r 平滑全隐、拉远恢复截图 r418b-near-betelgeuse-zoomin/zoomout；Esc 取消跟随 0.5s 淡出）；逐站 60 FPS 保持、控制台零错误；覆盖率 gate ≥90% 保持（距离窗口/球谐幅度纯函数单测 `stellarNearViewR418` 28 例，语句 98.48%）
...
- ✅ 纯逻辑：King (1966) profile 逆变换采样（`m13-profile.json` 参数：核半径/潮汐半径/浓度 c）替代现状均匀分布（新模块 `src/utils/m13Cluster.ts`——三维密度取 King 1962 经验轮廓解析去投影式即 King 1966 单质量模型标准近似登记；省 token 约定兑现：累积质量梯形积分 512 步 → 预计算 64 点逆 CDF 数值反查表 + 线性插值，每星 O(1) 非解析求逆；半质量半径 ≈0.121 r_t ≈5.2 pc 单测锚定；潮汐半径映射满视觉半径、核半径占 ~3% 登记；加载失败降级现状 rand² 分布 + P6 色板，`useM13Profile` hook 同 usePleiadesCatalog 范式）；恒星颜色按球状星团 HR 图近似分布（比例登记：老年红黄星族 90%——Teff 3,900–5,800 K u² 偏斜偏冷端；蓝离散星/水平支蓝端 10%——Teff 7,500–10,500 K 均匀）→ `blackbodyRGB`（R4-6 复用 + sRGB→线性）
- ✅ `GlobularCluster`（行号漂移 :2811，以符号名为准）与 R2-7 近观 +1,200 粒接入新分布（总预算不变：远观 420 + 近观 1,200，粒径/透明度/门控参数零变化；近观层新组件 `M13KingStarField` 消费预构建属性，降级路径保留 `ClusterNearStarField` 现状调用）；确定性种子（沿用现状 20260722/20260723，每星固定 6 次抽取两次构建逐字节一致）；R2-9 银晕程序化 29 星团**不在范围**（登记）
- ✅ 预览页注册 `?body=m13`（新组件 `M13ClusterPreview`，远观/近观两级同挂观感同源；滑杆 2 件粒径/亮度增益）；King 采样/颜色比例纯函数单测（`m13ClusterR419` 17 例：径向密度严格单调递减 + 采样粒子径向壳层体密度单调递减、半质量半径 0.121±0.015 断言、蓝星比例 10%±3%，m13Cluster.ts 语句覆盖 100%）

### 19.2 验收标准

- ✅ 近观：中心致密核 + 外围稀疏晕的真实球状星团密度梯度可辨（对照现状截图登记：CDP 阻断 m13-profile.json 降级 rand² 弥散分布 r419-scene-00-fallback-near vs King 致密亮核 r419-scene-01-king-near，同站同机位；预览页 r419-preview 系列）；红黄主色 + 少量蓝星点缀（预览页可辨）
- ✅ 巡游回归（昴星团↔M13↔邻站 ×3 强制 GC 后 JS 堆 50→50 MB 无泄漏、控制台零错误）、60 FPS（预览/近观/绕行/切换后全程 60）、覆盖率 gate ≥90% 保持（语句 98.49%，全量 2556 用例/134 套件）

## R4-20 WR 124 星风抛射壳 + 脉冲星射束体积化

### 20.1 需求

- ✅ WR 124：现有抛射壳 mesh 升级为小型体积层（64³，团块状抛射泡沫 + 径向速度场膨胀 uTime 驱动，M1-67 星云观感参考登记）；volume 池复用，近观激活。实现登记：`makeWr124Sampler`（球壳骨架 × 高频 fBm 阈值化团块泡沫，Hubble/JWST 2023 M1-67 公版图像观感参考、非逐结贴合；吸收通道恒零——Hα/[NII] 发射主导）；径向膨胀不烘焙进纹理——`NebulaVolumeMaterial` 新增 uExpandAmp/uExpandPeriodSec，raymarch 采样域按 1+amp·sin²(π·fract(t/period)) 齐次缩放（= v∝r 均匀膨胀流近似，幅度 0.14/周期 80s 与既有壳 mesh 动画同参；uExpandAmp 缺省 0 其余星云零回退）；包围盒 = 视觉尺寸 ×6.0（壳中面 1.8× 视觉半径与壳 mesh 1.9× 衔接）；壳 mesh 交叉淡出（`wr124ShellMeshFactor`）+ 光晕透射补偿（`wr124CoreBoostFactor` ×(1+0.35·vol01)，壳内腔近空压暗弱于蟹状登记）
- ✅ 脉冲星射束：`PulsarRemnant` 射束锥升级为轻量体积锥（沿锥轴密度衰减 + 边缘软化 + 扫描旋转保留），与 R4-16 蟹状体积层共存深度正确。实现登记：轴向 smoothstep 改幂律软衰减（`pulsarBeamAxial01` = (1−y)^1.6）；边缘软化取视线穿越圆锥截面弦长近似（弦长 ∝ |n·v| 精确几何关系 + 指数 1.3 艺术化软化档），替代原静态 uv.x 条纹——非逐步 raymarch 的表面着色近似体积积分（"轻量"取向登记）；DoubleSide 前后面各半弦长贡献；扫描旋转/脉冲节奏函数零改动（密集采样复验灯塔峰间隔 ≈2.4s）；补齐 logdepthbuf 兼容（附录 A §5 P6 遗留）；共存深度沿用 R4-16 登记（射束绘制先于体积合成 + crabCoreBoostFactor 透射补偿路径零改动）
- ✅ 两项均经 `useDetailLayer` 门控；预览页注册 `?body=wr-124`；密度塑形纯函数单测。实现差异登记：wr-124 预览条目 R4-6 已注册为 stellar-surface——本阶段升级为恒星 + 抛射壳组合条目（componentKey 维持 stellar-surface，PreviewScene 特例叠挂 NebulaVolumePreview：boxEdge 覆写 14 对齐恒星尺度、相机 3.2→9 外推；新增抛射壳密度/膨胀幅度滑杆两件，HUD 体积质量档行经 `previewHasVolumeLayer` 特例显示）；单测 `nebulaVolumeR420` 24 例（密度塑形/膨胀镜像/射束塑形/规格/预览注册）

### 20.2 验收标准

- ✅ WR 124 近观：抛射壳呈团块泡沫状立体膨胀（两帧对比登记）；脉冲星射束体积柔和无硬边锥。登记：预览页两帧膨胀量化（timeScale×4 半周期：外缘半径 449→473px、外环带亮度 +125%，截图 r420a-01/02）；主场景 25s 两帧对比（r420c-00/01）+ 绕行两角度团块立体视差（r420c-02/03）；射束侧视软边（r420c-05/06）
- ✅ 与蟹状站点连续巡游 LRU 正常；60 FPS 保持；覆盖率 gate ≥90% 保持。登记：WR 124↔蟹状往返 ×2 交替烘焙（64³ 51–61ms / 128³ 291–313ms 打点一致）、强制 GC 后 JS 堆 52→52 MB、全程 60 FPS、控制台零错误；覆盖率语句 98.51%

## R4-21 类星体 3C 273 近观升级（吸积盘 + 尘埃环面）

### 21.1 需求

- ✅ 近观细节层：中心吸积盘（复用 R4-12 盘着色逻辑的非透镜简化版——温度分布 + 束流着色，透镜 raymarch 不启用，登记）+ 外围尘埃环面（torus，小型体积或粒子环二选一登记）+ 现有相对论喷流保留联动。实现登记：a) 吸积盘为平面环形 mesh + R4-12 GLSL 镜像（温度剖面/开普勒 β/多普勒 δ³/引力红移 g³/差速条纹与黑体 LUT 常数均自 `utils/blackHoleLensing` 单点同源，`buildBlackbodyLutTexture` 复用）；透镜 raymarch 不启用登记——近观取盘远观尺度，弯折翻折像不可辨，平面盘即达验收观感；盘峰值色温压标 12,000 K（真实"大蓝包"~10⁴–10⁵ K，R4-12 压标先例）；b) 尘埃环面二选一取**粒子环**（2,400 粒确定性 FNV-1a，登记：volume 池容量 1 已被星云类 L3 巡游站高频占用、插入将致 LRU 反复逐出重烘焙，粒子环成本更低；points NormalBlending——尘埃为吸收介质，粒子堆叠维持暗红棕不被 additive 堆亮，目验调参登记）；c) 盘/环面平面 ⊥ 既有喷流轴（AGN 统一模型几何，Urry & Padovani 1995 登记）；几何尺度比例为可视化档（真实跨 3–5 量级登记）
- ✅ 光变（现有）保留；BLR 辉光过渡层（盘与环面之间弥散辉光 sprite）。实现登记：核心辉光近观按 `quasarCoreNearFactor` 减淡 ×0.45 让出盘视野（光变闪烁因子保留不回退，主场景亮度序列实测持续波动）；盘亮度同乘 quasarFlicker、BLR 辉光按 `quasarBlrOpacity` 光变联动呼吸；BLR 半边长 1.15 落于盘外缘 0.85 与环面主半径 1.45 之间（单测锚定层次次序）
- ✅ `useDetailLayer` 门控 + 预览页注册 `?body=quasar-3c273`。实现登记：kind='particles' 池（容量 1 与 R2-8 星系近观共池 LRU）、'lru-retain' L4 语义；进入阈值 = 河外飞往观察距离 ×1.5（`cameraFocus` 新导出 `EXTRAGALACTIC_VIEW_RADIUS_UNITS` 收敛原地字面量 300，单点同源）；预算 2,401 粒 ≤12,000 且低于星系近观峰值 → 全局峰值不变；预览页滑杆 4 件（束流强度/盘亮度/环面亮度/时间流速）+ 喷流同比例复用整体目验

### 21.2 验收标准

- ✅ 近观：盘（亮蓝白）→ BLR 辉光 → 尘埃环面（暗红棕）→ 喷流四层结构可辨（截图登记 r421 系列：预览页俯仰/侧视 + 主场景 L4 飞抵/绕行 2 角度）；光变不回退（核心亮度序列 201→236 luma 波动实测）；L4 巡游回归（M31 5/8 ↔ 类星体 8/8 particles 共池 LRU 交叉逐出往返 ×2，JS 堆 46→48 MB 稳定）；60 FPS（预览/近观/绕行/往返全程）；覆盖率 gate ≥90% 保持（语句 98.53%，quasarNearView.ts 100%，2606 用例/136 套件）

## R4-22 触须星系 N-body 烘焙潮汐尾

### 22.1 需求

- ✅ `scripts/bake-data/` 新增轻量受限三体/测试粒子模拟脚本（`antennae.ts` 接入 `npm run bake:data`；Toomre & Toomre 1972 图景：两 Plummer 软化点质心 G=1/M1=M2=1/ε=0.15 r_p + 各 2,796 ≤3,000 测试粒子盘——12 同心环 ∝ 半径配数 + Plummer 软化圆速度，双盘顺行倾角 60°/节点角相差 π 强潮汐尾条件；抛物线交会 e=1 登记：T&T 原文 Antennae 用 e≈0.5 椭圆，尾形态图景一致、两核末段分离偏快）：RK4 定步长 dt=0.01 自 t=−12（r≈10 r_p 入轨，Barker 方程反解）积至 t=+7，烘焙 10 个时间快照（域 8–12，均布 t∈[−2,+7] 近心点前后尾发育全程）的粒子位置到 `public/data/antennae.bin`（Float32 小端自述头 magic/version/S/N/nA，655.6 KB，public/data 总量 706.5 KB ≤5 MB 预算内；模拟参数/近似逐条登记于脚本文件头；mulberry32 确定性 seed=40384039，幂等实测两次运行 SHA-1 逐字节一致）
- ✅ `AntennaeGalaxies`（`ExtragalacticObjects.tsx`）近观层：消费烘焙快照（新组件 `Scene/AntennaeNearView.tsx`，主场景/预览页共用）——两核辉光 sprite（NGC 4038 暖橙 / NGC 4039 冷蓝，区分尾源盘的艺术化强调档登记）+ 双潮汐尾测试粒子 points，快照间线性插值随 simDays 缓慢演化。时间映射登记：快照全程 ↔ 600 Myr（T&T 尾发育时标量级；L4 时压下单程 30 s），simDays 按**三角波 ping-pong** 往返（真实交会为单向事件，取往返循环保证任意 simDays 演化持续可见且折返处插值连续——环绕 wrap 有末帧→首帧跳变故弃用登记）。插值实现登记：顶点属性双快照布局（position=seg / aPosB=seg+1 + shader mix(uMix)），每帧仅写 uniform、跨段才重传属性缓冲（渲染循环零分配）；场景缩放 1 r_p = 0.75× 基准半径（尾端伸展 ≈6.7× 与现状静态尾长同量级）；既有静态示意层近观减淡 85% 让位粒子结构（保留残影登记）；加载失败降级现状渲染（hook 返回 null 即不挂载，CDP 拦截产物实测零报错）
- ✅ `useDetailLayer({ kind:'starCatalog' })` 门控（容量 1 与 R4-17 昴星团共池，'lru-retain' L4 语义快速切回免重建；阈值与 resolveFocusTarget/R2-7 同源；预算登记：粒子 6,000+2 ≤12,000、GPU 按双快照实际布局 40 B/粒 > estimateGpuBytes 默认 28 B/粒）；预览页注册 `?body=antennae`（滑杆 2 件：时间流速 1 s=30 Myr 基准/粒径增益帧读 getter；数据未就绪线框占位）；脚本自校验（粒子数上限/无 NaN/坐标界 |r|<48/两核间距近心点前单调减后单调增/数值近心距偏差 <10%/两核两体能量 RK4 漂移 <2%·μ——"能量单调性宽松断言"落实为守恒断言登记，失败退出非零）+ 加载器单测（`validateAntennae` 二进制校验逐项拒绝路径 + 缓存/重试 + 实际产物集成断言，`bakedDataAntennae` 11 例；纯逻辑 `antennaeNearView` 19 例；两文件覆盖率 100%）。实现差异登记：tsconfig 全局新增 `allowImportingTsExtensions`（Node 原生 TS 执行烘焙脚本 ESM 需显式 .ts 扩展名，noEmit 下合法、build 复验无影响）

### 22.2 验收标准

- ✅ 近观：双核 + 两条弯曲潮汐尾立体可辨且缓慢演化（无头 Chrome Metal 1280×800：预览页构型与公版图像可对应——双核暖/冷 + 两条反对称弯曲尾，截图 r422-00 登记；绕行偏航/俯仰视差立体 r422-01/02；主场景 L4 飞抵近观粒子层激活暖/冷亮像素 1,595/2,921、12 s ≈240 Myr 帧差 5.83 演化可测，截图 r422s 系列；时间流速 8× 折返回交会前完整双盘态 r422-05 三角波映射生效）；快照插值无跳变（慢速连续 10 帧帧间差序列 2.42–4.82 平滑无尖峰登记）
- ✅ 烘焙产物幂等（两次运行 SHA-1 逐字节一致）；降级路径实测（CDP 拦截 antennae.bin：预览页降级占位 r422d-00 / 主场景静态渲染保持 r422d-01，均 60 FPS 零报错）；60 FPS（预览页默认/绕行/演化与主场景飞抵/绕行/进出全程实测 60，Esc 进出 ×3 强制 GC 后 JS 堆恒 47 MB 无泄漏）；覆盖率 gate ≥90% 保持（全量 2636 例/138 套件，语句 98.56%）

## R4-23 透镜星系团屏幕空间引力透镜弧

### 23.1 需求

- ✅ `LensingArcs`（`ExtragalacticObjects.tsx`）升级：近观时以屏幕空间折射近似呈现引力透镜——方案二选一并登记：**采用 a**（postprocessing 自定义 Effect，`ClusterLensingEffect.tsx`：团块质心 SIS 模型偏转屏幕 UV，Effect 基类最小实现单 mainUv fragment；集成实测无风险未降级 b——非卷积 Effect 自成 EffectPass 排在 Bloom 之前，Bloom 采样已透镜化帧缓冲几何一致，既有 Bloom 配置零改动）。实现登记：屏幕空间 UV 重采样近似仅对团块之后背景严格成立，前景/透镜体自身同被偏移（同方向 M87 光晕被拉成环状弧属该近似差异）；影响域窗 θ_E×2.5 外沿归零为实现性裁剪（真实 SIS 偏转全域恒 θ_E）；LensingArcs 同步升级——静态示意 ring 弧按效果强度减淡 75% 让位真折射弧（保留残影）、新增团块弥散光晕 + 6 个确定性背景源 sprite（世界系固定于团块之后，绕行视差下弧位置随视角一致）；修复登记：透镜挂载 setState 重渲染时库 Bloom 包装 JSON.stringify 已填充 ref（循环引用）崩溃——Bloom 元素 useMemo 固定引用规避
- ✅ SIS 偏转角纯函数（θ_E 爱因斯坦半径参数化）+ 单测（`utils/clusterLensing.ts` 覆盖率 100%，24 例：偏转恒 θ_E/透镜方程 β = θ − θ_E·θ̂ 环上归零环内翻转/切向放大率 r→θ_E 发散/角半径-UV 换算/域窗/uniform 组合/持有者/背景源确定性布局；θ_E = atan(300 单位/d) 随距离缩放远观自然消隐，真实 Abell 370 θ_E≈30″–40″ 压缩至近观 ~9.5° 可视化档登记，初值 420 因 M87 光晕放大过甚调优登记）；效果仅跟随 cluster-lensing 时激活（域判据复用 :409-416 现状——followBodyId/flyToBodyId 判据，`PostEffects.tsx` 命中挂载 + 0.5s 淡入淡出与统一细节层同源，焦点离开淡出完成即卸载）
- ✅ 预览页注册 `?body=cluster-lensing`（`dev/ClusterLensingPreview` 场景 + harness composer 挂入主场景同一 `ClusterLensingPass`；滑杆 3 件：爱因斯坦半径/透镜强度（0 关闭对照）/背景源亮度；参考网格同被偏转可直观检查折射域）

### 23.2 验收标准

- ✅ 近观：背景源被拉伸为切向弧/部分爱因斯坦环，弧位置随视角一致（截图登记 r423 系列：预览页强度 0 对照亮像素 4,854→25,114/环带 22,519、绕行偏航/俯仰一致；主场景 M87 光晕成部分爱因斯坦环 + 星点切向拉伸、同位置切焦点 M87 关闭对照帧差 77.2）；非跟随时零开销（透镜 pass 不挂载——性能对比登记：L4 非跟随基线 60 FPS = 跟随激活 60 FPS；Esc/重进 ×3 强制 GC 后 JS 堆 46→46 MB 无泄漏）
- ✅ 与 Bloom/体积层组合无伪影（Bloom 开/关 × 透镜开/关四态截图复验；透镜为纯颜色域 UV 重采样不读深度，与 log depth/体积层 RT 合成天然兼容）；60 FPS（预览/主场景飞抵/绕行/进出全程）；覆盖率 gate ≥90% 保持（语句 98.55%，2660 用例/139 套件）
...
- ✅ 全量 `npm test` / `npm run type-check` / `npm run lint` / `npm run build` 全绿；覆盖率 gate ≥90% 确认（2,660 用例/139 套件，语句 98.55%；`npm run bake:data` 幂等复核——两次运行产物 shasum 逐字节一致，`public/data/` 总量 706.5 KB）
- ✅ 无头 Chrome 端到端（Metal 1280×800，r424 系列截图/日志归档）：L3 14 站 + L4 8 站巡游一整圈（实现差异登记：需求文本"L3 15 站"含 sun，sun 已于 R4 期间因与 heliopause 语境重复移出巡游序列——`cycleScopes.ts` 文件头既有登记，实际 14 站），逐站近观截图 + FPS + GC 后 JS 堆登记——全部 22 站 60 FPS（无一跌破 55、未触发降档）、堆 46→52 MB 平稳、控制台零错误、逐站细节层持有者核对正确；连续切换 15 个细节层目标（四类交叉含 L3↔L4 跨域）后堆稳定——以生产构建为准登记：`out/` 静态产物 L3 巡游 ×3 轮轮末 GC 堆 30.9→31.4→31.1 MB 平台稳定；实现差异登记：dev 模式下切换伴随 ~0.1–0.7 MB/次堆增长，分配采样取证为 React dev/drei Html root churn 保留（jsxDEV 元素树 + createRoot 监听器），细节层自身资源无保留（快照 Float32Array 不在保留栈、空转 300 s 堆零漂移、GPU 出账归零），登记为 dev-only 现象；四类细节层（particles/volume/lensing/starCatalog）LRU 交叉逐出验证全 PASS（volume：orion→ring→crab 依次逐出；lensing：sgr-a-star↔cygnus-x1；starCatalog：pleiades↔antennae-galaxies；各池全程 holder ≤ 容量 1，lru-retain 保留层不被跨类切换误逐）
- ✅ 预览页全部 19 个注册对象逐一截图归档（亮像素/FPS 抽查全部正常渲染，重负载条目 60 FPS）；自适应降级强制触发验证——orion-nebula 质量档滑杆 0 自动（high 64 步/RT 100%）→ 1 强制 low（32 步/RT 50%）→ 3 强制 high → 0 自动恢复，HUD 与实际渲染一致
- ✅ `CHANGELOG.md` `[Unreleased]` 查漏补全全部 R4 条目（R4-1～R4-24 逐阶段含实测数据，本阶段新增 R4-24 验收条目）；`IMPROVEMENT_REQUIREMENTS_4.md` 全部 🔲 回写 ✅（实现差异逐条登记于各阶段验收标注），文档版本 1.2 → 1.3；REQUIREMENTS.md 受影响小节同步（§4.3/§4.4 天体细节呈现补 R4 近观细节层机制说明）
- ✅ 性能总账登记（逐项对照附录 A 预算全部不超）：单目标近观粒子峰值 L3 = M13 近观 +1,200（全局峰值 ≤16,200 ≤ 20,000 粒预算，`nearView.ts` 登记）/ L4 = M31 近观 9,850 ≤ 单目标 12,000 上限；体积纹理常驻显存最大 128³×2ch = 4 MB（≤128³ 约束；volume 池容量 1 → 常驻至多一份，E2E 实测 GPU 估算峰值 4.2 MB、四池共存最坏理论值 ≈4.9 MB ≪ 64 MB `DETAIL_GPU_BUDGET_BYTES`）；`public/data/` 资产总量 706.5 KB ≤ 5 MB

### 24.2 验收标准

- ✅ 上述全部完成并登记；无既有功能回归（R2/R3 关键验收点抽查各 1 项全 PASS，r424/regress 截图归档）：R3-1 巡游域切换（L3 巡游落银河系序列成员、切 L4 后巡游落宇宙序列成员不串台）；事件域隔离（L3 触发超新星激活 → 切 L1 离域按宽限丢弃 activeSupernova=null）；卫星角尺寸钳制（ISS 跟随抵达/极近推进两档截图人工判定 ≈2–3% 屏高 ≤ 10% 钳制上限；登记：滚轮不可压过跟随相机最小距离下限，极近淡出档由 `satelliteProximityFade01` 单测 100% 覆盖）；合并演化预览（激活/演化推进 0.250→0.585/恢复回退 3,168 Myr 快进、残差 30 Myr 为 L4 自然时钟推进非回归）。附登记：验收基础设施——dev 环境暴露 `window.__simStore`/`window.__detailLayerDebug` 供 CDP 脚本读写状态与核对细节层注册表（生产构建条件恒假摇树剔除，运行时零影响）
