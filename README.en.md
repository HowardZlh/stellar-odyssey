...
> audio. A science-data-driven, immersive tour of the cosmos.

<p align="center">
  <a href="https://stellar.guushu.com"><strong>🚀 Try it live → stellar.guushu.com</strong></a>
</p>
...
> see the highlights below, or [just start playing](https://stellar.guushu.com).
...
**Online**: just visit [stellar.guushu.com](https://stellar.guushu.com) — nothing to install.
...
Once open: **scroll to zoom**, press `1`–`4` to switch levels, and **click any celestial body** to inspect and fly to it. Full controls are documented in [docs/controls.md](docs/controls.md) (Chinese).

> A modern browser with WebGL 2 is recommended (Chrome / Edge / Firefox / Safari).

## 🛠 Tech Stack

Next.js 14 · React 18 · TypeScript (strict) · Three.js / React Three Fiber · custom GLSL shaders (logarithmic depth buffer, raymarched volumes, gravitational lensing) · Zustand · Web Audio API · Tailwind CSS · Jest + React Testing Library (2,951 tests / 154 suites, coverage gate ≥90%) · offline data-baking pipeline (Gaia DR3 / SIMBAD / 2MRS / DSS2 → ~2.5 MB static artifacts, zero runtime external requests)

## 🔬 Scientific Integrity

All astrophysical parameters are based on real scientific data (NASA JPL, SIMBAD, NED, USGS Astrogeology, etc., with sources registered in code and in-app info panels); orbits use Kepler's laws; **every artistic liberty (size exaggeration, time compression, sound design…) is explicitly registered** in-app and in code comments. See [docs/science-notes.md](docs/science-notes.md) (Chinese).
...
Full per-asset details, licenses, and registration notes are in the [Chinese README's Attribution section](./README.md#素材许可attribution).
