...
| **Bottom · Onboarding guide** | A one-line controls hint on first visit (auto-dismisses after 12 s); click "?" to reopen the full guide (shortcuts, language switching, and scientific-accuracy notes) |
...
1. **Start with the Solar System overview** -- the app opens in the Solar System view (L2), not following any body, with the eight planets moving along real Keplerian orbits (planet positions match the actual current date). Press `1` to switch to the Planet view and fly to Earth, which the camera then follows: drag to look around at continents, oceans, cloud layers, and city lights on the night side; press `]` to tour the Earth system: Tiangong space station → ISS → Hubble → geostationary satellites → the Moon.
2. **Zoom out to the Solar System** -- press `2` to return to the Solar System view. Drag the speed slider to accelerate time and watch Mercury race around and Halley's Comet whip past perihelion.
