...
**用户反馈**：控制面板功能选项有点多需要整理——只有全局的功能选项才能在不同视角下同时显示并可用，其他功能选项应该只有在对应的视角下才显示并可用（例：太阳内部剖面只在太阳系视角、垂直展开只在银河系视角、卫星轨道线只在行星视角）。

**现状核实**（2026-07 代码调研，行号漂移以符号名为准）：

| 选项 | 实际生效层级（消费点） | 现状 |
|---|---|---|
| 轨道线（O） | L1/L2 行星/矮行星/彗星轨道（SolarSystem.tsx）+ L4 卫星星系轨道（Universe.tsx :588-593） | 跨视角，属全局 |
| 天体标签（L） | L1–L4 全层级约 20 处消费 | 属全局 |
| 卫星轨道线 | Moon.tsx 唯一消费（:358），L1/L2 渲染；卫星标签本就硬门控 L1（:76） | 任何视角均显示复选框 |
| You are here 标记 | Galaxy.tsx 唯一消费（:842-892），L3 银河系组（L4 平台区仍可见） | 同上 |
| 速度矢量箭头 | Universe.tsx 唯一消费（:792-822），仅 L4（inRange > 3.05） | 同上 |
| 垂直展开（V）+ 增益滑块 | Galaxy.tsx / SpecialBodies.tsx，L3 银河系组 | 同上 |
| 真实比例模式 | Sun/Planet/Moon 尺寸（L1/L2）+ Galaxy 太阳振荡增益归 1（L3） | 跨视角，属全局 |
...
**用户确认项**（2026-07 需求澄清）：
1. 视角专属选项域外**隐藏**（非置灰）——含动态事件演示按钮（推翻 R2-4"置灰 + tooltip"确认方案，行为变更登记）；
2. **真实比例模式归全局**（跨 L1–L3 生效的科学口径开关）；
3. **卫星轨道线归 L1**（与卫星标签既有 L1 硬门控对齐；L2 下卫星轨道极小几乎不可辨，登记）；
4. **V 快捷键补 L3 门控**（与 G 键一致，非 L3 按下无效；修正 R3-6"V 键无层级门控"登记差异）；
5. **域外已开启状态与场景效果保留**（只整理 UI 显示、零渲染行为变更；如 V 开启切 L4 银河系仍呈椭球——R3-7 登记语义不动，回 L3 复选框仍勾选）。
...
- ✅ 新建 `src/utils/panelScopes.ts`：导出面板选项 id → 可见视角集合的注册表 `PANEL_OPTION_SCOPES` 与判定纯函数 `panelOptionVisible(optionId, viewLevel)`（未知 optionId 抛 RangeError）；归类：
  ...
  - **L2 太阳系视角**：sunCutaway（太阳内部剖面）
  - **L3 银河系视角**：galacticFrame（银心固定参考系区块）/ verticalExpand（垂直展开 + 增益滑块）/ youAreHere（You are here 标记）/ supernovaDemo（超新星演示）
