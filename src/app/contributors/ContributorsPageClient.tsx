...
export default function ContributorsPage(): JSX.Element {
  ...
  // M2 动态名单（D-z4，统一收敛至 useContributorsRoster 共享 hook）：
  // 静态 DONORS + 远程 /api/contributors 合并，拉取失败静默降级；
  // 匿名展示名随 locale 切换，匿名星布点随之重派生——页面级画布数据流
  // 更新，非主场景重建，可接受（登记）。C1 布点一次完成（渲染循环零重算）。
