...
/**
 * 日全食实验室场景页 `/lab/solar-eclipse`（E-M2-2，静态导出）
 *
 * 场景组件经 `next/dynamic` 动态 import（`ssr: false` + 加载进度提示）——
 * 实验室场景（three/R3F 场景图 + 后期）打进独立 chunk，仅在进入本页时
 * 加载，主页首屏 bundle 零增大（流星雨范式，§0.1）。
 * 星历 JSON 懒加载在场景组件内（useSolarEclipses），两级加载提示。
 */
...
const SolarEclipseLab = dynamic(
  () => import('@/components/Lab/SolarEclipseLab').then((m) => m.SolarEclipseLab),
  { ssr: false, loading: () => <SceneLoading /> },
);

export default function SolarEclipseLabPage(): JSX.Element {
  // 独立页面同样按 ?lang= > localStorage > zh 初始化（donate/contributors 同范式）
  useLocaleInit();
  return <SolarEclipseLab />;
}
