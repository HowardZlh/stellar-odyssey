/**
 * 矮行星数据测试（需求 3.1.1 / P5 §3.1：IAU 官方 5 颗矮行星）
 */
...
import { ASTEROID_BELT } from '@/data/smallBodies';

describe('矮行星清单（P5 扩展为 5 员）', () => {
  it('共 5 员：谷神星 + 冥王星 + 阋神星 + 鸟神星 + 妊神星', () => {
  ...
  it('id 唯一且按半长轴升序排列（谷神星最内、阋神星最外）', () => {
    const ids = DWARF_PLANETS.map((d) => d.id);
    expect(new Set(ids).size).toBe(ids.length);
    expect(DWARF_PLANETS[0]).toBe(CERES);
    for (let i = 1; i < DWARF_PLANETS.length; i += 1) {
      expect(DWARF_PLANETS[i].orbit.semiMajorAxisAu).toBeGreaterThan(
        DWARF_PLANETS[i - 1].orbit.semiMajorAxisAu,
      );
    }
  });
  ...
  it('开普勒第三定律自洽（T ≈ a^1.5，±2%）', () => {
    for (const d of [CERES, ...OTHER_DWARF_PLANETS]) {
...
describe('谷神星（Ceres）科学性（P5 §3.1，NASA JPL SBDB / Dawn）', () => {
  it('轨道要素：a ≈ 2.77 AU、e ≈ 0.0785、i ≈ 10.59°', () => {
    expect(CERES.orbit.semiMajorAxisAu).toBeCloseTo(2.77, 1);
    expect(CERES.orbit.eccentricity).toBeCloseTo(0.0785, 3);
    expect(CERES.orbit.inclinationDeg).toBeCloseTo(10.59, 2);
  });

  it('位于小行星带内（2.2–3.2 AU）', () => {
    expect(CERES.orbit.semiMajorAxisAu).toBeGreaterThan(ASTEROID_BELT.innerAu);
    expect(CERES.orbit.semiMajorAxisAu).toBeLessThan(ASTEROID_BELT.outerAu);
  });

  it('物理参数：半径 469.7 km、自转 9.07 h、质量 9.38×10²⁰ kg、公转 4.60 年', () => {
    expect(CERES.radiusKm).toBeCloseTo(469.7, 1);
    expect(CERES.rotation.siderealPeriodHours).toBeCloseTo(9.07, 1);
    expect(CERES.massKg!).toBeCloseTo(9.38e20, -19);
    expect(CERES.orbitalPeriodYears).toBeCloseTo(4.6, 1);
  });

  it('数据来源标注 JPL SBDB 与 Dawn 任务', () => {
    expect(CERES.dataSource).toContain('Small-Body Database');
    expect(CERES.dataSource).toContain('Dawn');
  });

  it('是最小的矮行星（半径小于其余四颗）', () => {
    for (const d of DWARF_PLANETS) {
      if (d.id !== 'ceres') {
        expect(CERES.radiusKm).toBeLessThan(d.radiusKm);
      }
    }
  });
});
