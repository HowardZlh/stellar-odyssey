...
export const zh = {
   ...
   * 爱发电（推荐）→支付宝→面包多→Ko-fi→预留位，爱发电为推荐面板、支付宝为
   * 引导面板跳 /unlock；微信赞赏码已下线）。
  ...
  donate: {
    ...
    intro:
      "支持项目即可解锁高级内容：推荐通过爱发电支持——支付后凭订单号在解锁页自动兑换，即时解锁近观细节层与巡游演示；也可在解锁页支付宝扫码，支付成功后自动发放解锁凭证。昵称与留言（均可选）将记入贡献者名单与贡献者宇宙。项目的全部源代码保持开源，你的支持将用于持续开发与域名维护，让它保持免费、无广告、开源。",
    platformsSection: "支持通道",
    platformAvailable: "前往支持",
    platformComingSoon: "预留位 · 即将开通",
    /** 爱发电推荐面板（站外购买，回解锁页凭订单号自动兑换） */
    afdianGuide:
      "推荐渠道：前往爱发电按档位金额购买（周卡/年卡为商品，月卡为订阅方案，需注册爱发电账号），支付完成后回到解锁页粘贴订单号即可自动兑换、即时解锁；昵称与留言记入贡献者名单。",
    afdianCta: "前往爱发电支持",
    afdianRedeemCta: "去解锁页兑换订单号",
    /** 支付宝引导面板（付款 modal 只在 /unlock，本页仅引导跳转） */
    alipayGuide:
      "在解锁页点击档位卡片生成付款码，支付成功后自动发放解锁 token 并即时解锁，无需注册账号；昵称自动记入贡献者名单。",
    alipayCta: "前往解锁页扫码支付",
    /** 备选通道卡片说明行 */
  ...
  contributors: {
    ...
    /** M2 动态名单（D-z4）：空昵称展示名 + 注册表外平台展示名（支付宝短名 / 已下线的微信赞赏码历史记录） */
    anonymous: "匿名用户",
    platformAlipay: "支付宝",
    platformWechat: "微信赞赏码",
  },
  /**
   * 解锁页（/unlock，U3；Z 迭代 M3 起统一"支持即解锁"口径）。
   * 本页为明码标价对价口径（允许"支付 ¥X 解锁 Y 天"承诺式表述）；
   * 渠道顺序：爱发电（推荐）→ 支付宝 → 面包多（备选）→ Ko-fi（海外备选）；
   * 微信赞赏码已下线。
   */
  unlock: {
    title: "支持者解锁",
    ...
    contributorsPendingNote:
      "支付宝为即时上榜；爱发电/面包多/Ko-fi 等渠道稍后上榜。",
    /** 权益状态区 */
    statusSection: "我的权益",
    ...
    /** 四通道购买与兑换区（渠道重排：爱发电（推荐）→支付宝→面包多→Ko-fi） */
    channelsSection: "购买与兑换",
    alipayChannelTitle: "支付宝扫码支付（支付后自动发码即时解锁）",
    alipayChannelGuide:
    ...
    afdianTitle: "爱发电（推荐 · 订单号自动兑换）",
    afdianGuide:
      "前往爱发电按档位金额购买（周卡/年卡为商品，月卡为订阅方案，需注册爱发电账号），支付完成后在下方粘贴订单号即可自动兑换、即时解锁。",
    afdianLink: "前往爱发电购买",
    ...
    /**
     * Ko-fi 人工渠道兑换邮件正文（mailto 预填，utils/redeemMail 拼装）：
  ...
  helpHint: {
    ...
    /** G3 帮助面板科学性说明分节标题（首屏不再展示长文，见 HelpHint 头注） */
    disclaimerTitle: "科学性与艺术化说明",
  ...
  kiosk: {
    ...
    exit: "退出",
    exitAria: "退出展馆模式",
    /** G2 巡游门控回落一次性说明（tour=galaxy/universe 且未解锁时展示） */
    gateFallback:
      "银河系/宇宙巡游为支持者专属，未解锁时已自动改为太阳系巡游路线",
    gateFallbackCloseAria: "关闭巡游路线说明",
  },
  controlPanel: {
    title: "星海奥德赛",
